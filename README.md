...
Password Space Blog is a Django 4.2 web application focused on practical cybersecurity guidance, especially password hygiene and account protection.
It combines educational articles with a community idea pipeline where visitors can submit stories and suggestions for future content.
...
- `/ideas/my/` My Ideas (authorisation required)
...
- TESTING NOTES — Complete test coverage is included in this file under Testing and Validation heading including 18 test cases, security measures verified, and testing infrastructure.
...
Password Space Blog maintains comprehensive test coverage across all critical functionality to ensure system reliability and security.
This document outlines the test suite, providing stakeholders with visibility into the application's stability without requiring deep dives into source code.

## Test Summary

| Category             | Test Class              | Test Count | Coverage                               |
|----------------------|------------------------ |------------|----------------------------------------|
| Authentication       | Registration Api Tests  | 6          | Registration, validation, security     |
| Login Security       | Login Api Tests         | 2          | Multi-factor validation                |
| Authorisation UI/UX  | Auth Page Routing Tests | 2          | Password reset flows                   |
| Password Reset       | Password Reset Api Tests| 3          | Email delivery, token handling         |
| Idea Management      | Idea Page Tests         | 4          | Access control, prefilling, privacy    |
| Content Publishing   | Post Publishing And Editor Tests | 1+ | Status filtering, editorial workflows |
...
5. Registration Creates User with Hashed Password
   - Verifies passwords are hashed (never stored in plain text)
   - Confirms Argon2 hasher integration (`$` prefix in hash)
   - Creates associated User Contact Profile automatically
...
- Telephone number captured in User Contact Profile
...
1. Login Rejects Mismatched Telephone
   ...
   - Returns HTTP 401 Unauthorised on mismatch
   - Prevents unauthorised access even with correct password
   - Status: Passing
...
- Format normalisation for telephone numbers
...
2. Password Reset Request Supports Username/Email (Aged Accounts)
   - Handles accounts with email-as-username (aged compatibility)
...
2. Idea Form Prefills Email from Username (Blank Email Edge Case)
   - Handles aged accounts where email field is blank
...
### Authorisation Page Routing (2 tests)
...
- Tests access control and authorisation
...
```python
# Aged accounts with email as username
...
2. Multi-Factor Login: Telephone verification prevents unauthorised access
...
1. Edge Case Coverage: Aged accounts, blank fields, format variations
...
2. Group by feature in dedicated Test Case classes
...
Enforce Argon2 Hashing: Confirm Argon2 Password Hasher remains prioritised at the top of your PASSWORD_HASHERS array in settings.py to protect user identities with high memory-hard resistance. Enforce Length Constraints: Verify that your global AUTH_PASSWORD_VALIDATORS configurations explicitly override standard rules to mandate a strict 12-character minimum length for all accounts.

#### Authentication & API Flow Protections
Defensive Token Protection: Verify that your Simple JWT configuration settings establish defensive, short-lived expiration windows for all access tokens handling your /API/* authentication endpoints. Isolate Environment Workflows: Keep local development environments mapped to the terminal console email backend (django.core.mail.backends.console.EmailBackend), whilst reserving real SMTP configurations exclusively for live staging.
...
- Cloud platform setup: Step 1
- Environment variables: Step 4
- Production database setup: Step 5
- Migrations and release execution: Step 6

### Step 1: Pre-Deployment Configuration

Initialise App and Database via CLI. Open your local terminal, log into Heroku dashboard, and create your application:
...
### Step 2: Deployment Pre-Flight Sanity Test
...
### Step 3: Update Project Settings for Production

Modify your settings.py file to protect your application from vulnerability risks:

- Set DEBUG = False to prevent raw error logs and stack traces from displaying to public users.
- Generate a strong, unique value for SECRET_KEY and store it inside environment variables.
- Add your custom domain or hosting URL to the ALLOWED_HOSTS list.
...
### Step 5: Managed PostgreSQL Provisioning
...
### Step 6: Runtime Execution & Pipeline Automation
...
```bash
gunicorn password_app.wsgi --log-file -
```

### Step 7: Configure a Production WSGI or ASGI Server

Django's built-in runserver command is strictly intended for local development and lacks the capability to handle concurrent production traffic.

WSGI Servers (Synchronous Apps): This project used a production server named Gunicorn or uWSGI to run the application callable defined in your project's wsgi.py file.

### Step 8: Handle Static and Media Files

In production, Django does not serve static assets (CSS, JavaScript, images) on its own.

Define a target directory by specifying a STATIC_ROOT path in settings.py.

Run python manage.py collectstatic to aggregate all app assets into that single folder.

This project configured a lightweight Python utility like WhiteNoise to let Django serve compressed assets directly. Static files may be offloaded to an external storage service like Amazon S3 or a Content Delivery Network. (CDN).

### Step 9: Manage System Dependencies

Generate an updated dependencies file by executing pip freeze > requirements.txt in your local terminal.

Ensure the target production server reads this file to match and install your exact local environment framework versions.

### Step 10: Deploy to a Hosting Environment

Heroku was selected as a hosting infrastructure tier for this project based on technical needs. This was to allow integration of Heroku directly to this project GitHub repository.

Heroku automatically triggers a fresh application build and launches the WSGI server every time new code is pushed to this project`s repository.

Containerised Deployment: This was carried out using Profile with release and web processes as an alternative path not a required step for this project. Procfile was used to wrap core application framework and environmental dependencies together. 

### Step 11: Post-Deployment Initialisation

Run these final operational commands using your cloud platform's Remote Console.
...
Never reuse your development secret key or hardcode secrets in your codebase.
You can instantly generate a completely random, cryptographically secure 50-character string by running this command in your local terminal:
...
```bash
heroku config:set SECRET_KEY="your_generated_output_here"
```

### Production settings.py Integration Check

To ensure your Django site dynamically reads the Heroku database and environment variables without breaking your local SQLite setup, verify your settings.py contains the following configuration:
...
Ensure your system uses the compressed, manifest-backed asset cache loader.
This engine appends unique hash strings to static file names during build execution, forcing browser caches to clear instantly on new site updates:

```python
# Absolute file system path to the directory where collectstatic collects assets for production
